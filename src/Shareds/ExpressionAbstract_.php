...
class ExpressionAbstract_
{
    ...
    for( $i=count( $this->tokens ) - 1; $i>=0; $i-- ){
      if( strpos( $this->tokens[$i], 'function' ) !== false ){
        if( isset( $this->cacheMethod ) === false ){
          ...
          calcTimer( "Search Function Name ExpressionWhere::Origins" );
        }
      }
      ...
      if( strpos( $this->tokens[$i], 'class' ) !== false ){
        if(isset( $this->cacheClass ) === false ){
          ...
          calcTimer( "Search Classe Name ExpressionWhere::Origins" );
        }
      }      
    }
  }
  ...
  public function analysisLexicalCache(
    ...
    if( file_exists( $this->cacheFile())){
      ...
      calcTimer( "Require File ExpressionWhere::Cache" );
  ...
  public function analysisLexical(
    ...
    calcTimer( "Criar Listagem de ExpressionWhere::Cache" );
  }  
