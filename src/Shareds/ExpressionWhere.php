...
extends ExpressionAbstract
{
          ...
          if($contextsALeftJ[0] === T_EXP_FIELD){
            if($contextsALeftI[1] === $contextsALeftJ[1]){
              if($contextsALeftI[2] === $contextsALeftJ[2]){
                if(in_array( $contextsALeftI[3], [ "Datetime" ])){
                  if(in_array( $contextsALeftJ[3], [ "Datetime" ])){
  ...
  public function getParserInitial(
    ...
    $tokens = $this->getParser( T_EXP_INITIAL, $scopes, $tokens );
    return $this->getParserImplode( $tokens );    
  }  
